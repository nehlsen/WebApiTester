...
create table plan_execution_record (created timestamp(6), end_time_epoch_millis bigint not null, start_time_epoch_millis bigint not null, updated timestamp(6), plan_uuid uuid, uuid uuid not null, primary key (uuid));
...
alter table if exists plan_execution_record add constraint FKfda3eijrbkd7cqcckrctpebph foreign key (plan_uuid) references plan;
